...
LOAD DATA INFILE 'C:\\ProgramData\\MySQL\\MySQL Server 8.0\\Uploads\\amazon_prime_data.csv'
INTO TABLE amazon_prime_data
...
SELECT DISTINCT country FROM amazon_prime_data;
...
SELECT
    type,
    COUNT(*) AS count,
    ROUND(COUNT() / (SELECT COUNT() FROM amazon_prime_data) * 100, 2) AS percentage
FROM
    amazon_prime_data
GROUP BY
    type;
